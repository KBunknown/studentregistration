...
function Blob() {
  const mesh = useRef<React.RefObject<THREE.Mesh>>(null as any);
  ...
  const uniforms = useMemo(
    () => ({
      ...
      u_color: { value: new Color("#3b82f6") },
  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      (mesh.current as THREE.Mesh).position.copy(currentPosition.current);
    }
  });
...
export function MeshBlobBg() {
  return (
    <div className="fixed inset-0 z-0" style={{ pointerEvents: "none" }}>
      <Canvas
        camera={{ position: [0, 0, 8], fov: 10 }}
