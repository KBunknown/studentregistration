  ...
  return (
    <AdminShell title="Graduated Students">
      <p className="text-sm font-medium text-muted-foreground">
        {list.length} graduated students on record.
      </p>
      {list.length === 0 ? (
        <div className="mt-6 glass-panel rounded-xl p-12 text-center shadow-card">
          <div className="mx-auto grid h-16 w-16 place-items-center rounded-xl bg-primary-soft text-primary-deep">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Mark students as graduated from their individual profiles.
          </p>
        </div>
      ) : (
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {list.map((r) => (
            <Link
              ...
              params={{ id: r.id }}
              className="group block glass-panel rounded-xl p-5 shadow-sm transition hover:border-primary/40 hover:shadow-card"
            >
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs font-medium text-muted-foreground mt-0.5">
                    Class of {r.graduationYear} · {r.program}
                  </p>
                </div>
              </div>
            </Link>
