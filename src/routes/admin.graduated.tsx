...
function Graduated() {
  ...
  return (
    <AdminShell title="Graduated Students">
      ...
      {list.length === 0 ? (
        <div className="mt-6 rounded-xl border border-border bg-white p-12 text-center shadow-card">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {list.map((r) => (
            <Link key={r.id} to="/admin/students/$id" params={{ id: r.id }} className="group block rounded-xl border border-border bg-white p-5 shadow-sm transition hover:border-primary/40 hover:shadow-card">
