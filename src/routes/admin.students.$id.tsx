  ...
  return (
    <AdminShell title="Student Profile">
      <div className="flex items-center justify-between">
        <Link to="/admin/registrations" className="inline-flex items-center gap-1.5 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">
          <ArrowLeft className="h-4 w-4" /> Back to registrations
        </Link>
        <Link to="/admin/students/$id/edit" params={{ id: r.id }} className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-primary-deep">
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4 sm:flex sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-5">
            ...
            <div className="min-w-0">
              <h2 className="truncate font-heading text-2xl font-bold text-primary-navy">{r.fullName}</h2>
              <p className="mt-1 text-sm font-medium text-muted-foreground">Index #{r.index}</p>
            </div>
          </div>
          <span className={"inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold shadow-sm " + (r.graduated ? "border-success/30 bg-success/10 text-success" : "border-primary/30 bg-primary-soft text-primary-deep")}>
            <GraduationCap className="h-4 w-4" /> {r.graduated ? "Graduated" : "Active"}
          </span>
        </div>

        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          <div className="flex items-center gap-3 text-sm text-foreground"><div className="grid h-8 w-8 place-items-center rounded bg-muted"><Mail className="h-4 w-4 text-muted-foreground" /></div> <span className="font-medium">{r.email}</span></div>
          <div className="flex items-center gap-3 text-sm text-foreground"><div className="grid h-8 w-8 place-items-center rounded bg-muted"><Phone className="h-4 w-4 text-muted-foreground" /></div> <span className="font-medium">{r.phoneCode} {r.phone}</span></div>
          <div className="flex items-center gap-3 text-sm text-foreground"><div className="grid h-8 w-8 place-items-center rounded bg-muted"><MessageCircle className="h-4 w-4 text-muted-foreground" /></div> <span className="font-medium">{r.whatsappCode} {r.whatsapp}</span></div>
          <div className="flex items-center gap-3 text-sm text-foreground"><div className="grid h-8 w-8 place-items-center rounded bg-muted"><MapPin className="h-4 w-4 text-muted-foreground" /></div> <span className="font-medium">{r.country}</span></div>
        </div>
      </div>

      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Field label="Program" value={r.program === "Other Program" ? `${r.otherProgram} (Other)` : r.program} />
        ...
        <div className="mt-6 flex items-start gap-3 rounded-xl border border-warning/30 bg-warning/5 p-5 text-sm text-warning-foreground">
          <div>
            ...
            <p className="mt-1 text-warning/80">
              ...
              <Link to="/admin/students/$id" params={{ id: r.duplicateOf }} className="font-semibold underline underline-offset-4 hover:text-warning">
                {r.duplicateOf}
              </Link>.
            </p>
          </div>
